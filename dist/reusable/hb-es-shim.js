    ...
    Date: {
        fromISO: (function () {
            ...
            var noOffset = function (input) {
                ...
                var offsetString = input.slice(-5), offset = parseInt(offsetString, 10) / 100;
            ...
            return function (input) {
                var day, timezone, regExp = /^(\d{4}\-\d\d\-\d\d([tT][\d:\.]*)?)([zZ]|([+\-])(\d\d):?(\d\d))?$/, splitted = regExp.exec(input) || [];
                if (splitted[1]) {
                    ...
                    if (splitted[5]) {
                        ...
                        if (splitted[6])
                            timezone += parseInt(splitted[6], 10);
                        if (splitted[4] == "+")
                            timezone *= -1;
                        if (timezone)
...
//# sourceMappingURL=hb-es-shim.js.map